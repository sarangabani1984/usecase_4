...
def main():
   ...
   gcs_df = spark.read.csv("gs://temp-buck2024/dataset/custs",mode='dropmalformed',schema=custstructtype1)
   gcs_df.show(10)
   ...
   print(curts)
   gcs_df.repartition(2).write.json("gs://sarang-data-store/dataset/cust_output_json_"+curts)
   ...
   curts = spark.createDataFrame([1], IntegerType()).withColumn("curts", current_timestamp()).select(date_format(col("curts"), "yyyyMMddHHmmSS")).first()[0]
   print(curts)
   gcs_df.repartition(2).write.mode("overwrite").csv("gs://sarang-data-store/dataset/cust_csv")
   print("gcs Write Completed Successfully")
